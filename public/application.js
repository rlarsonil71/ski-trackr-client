...
/***/ (function(module, exports, __webpack_require__) {
	...
	var tripEvents = __webpack_require__(14);
	...
	$(function () {
	  ...
	  );$('#log-my-trip-modal').on('click', function () {
	    $('#tempLogMyTripError').html('');
	  });
	  $('#log-my-trip-modal').on('click', function () {
	    $('#log-my-trip').find('input:text, select, textarea').val('');
	  });

	  $('.setUpdateTripBtn').on('click', function () {
	    $('#tempUpdateMyTripError').html('');
	  });
	  $('.setUpdateTripBtn').on('click', function () {
	    $('#handlebars-update-my-trip-form').find('input:text, select, textarea').val('');
	  }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var authUserText = __webpack_require__(13);
	...
	var changePasswordSuccess = function changePasswordSuccess(ajaxResponse) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var getFormFields = __webpack_require__(9);
	var tripApi = __webpack_require__(15);
	var tripUi = __webpack_require__(16);
	var tripUserText = __webpack_require__(39);

	var store = __webpack_require__(11);

	var onCreateTrip = function onCreateTrip(event) {
	  event.preventDefault
	  ...
	  // If the first character of ALL `trip` fields have data, check for valid entered date
	  );if (data.trip.tripDate[0] && data.trip.resort[0] && data.trip.equipmentType[0] && data.trip.conditions[0] && data.trip.numberOfRuns[0] && data.trip.favoriteRun[0] && data.trip.comments[0]) {
	    ...
	    );var tripEnteredDate = new Date(data.trip.tripDate).toJSON().slice(0, 10).replace(/-/g, '/');

	    if (tripEnteredDate > todayDate) {
	      // Can't create a trip after TODAY
	    ...
	    if (data.trip.numberOfRuns > 60) {
	      // User entered a number for `numberOfRuns` that existed the max, log error
	      //  and return.
	      tripUserText.logMyTripNumberOfMaxRunsReachedError();
	      return;
	    }
	    ...
	    ();tripApi.createTrip(data).then(tripUi.createTripSuccess).catch(tripUi.createTripFailure);
	  } else {
	...
	var onIndexTrips = function onIndexTrips(event) {
	  event.preventDefault
	  ...
	  // Don't need to use data object here!
	  ();tripApi.indexTrips().then(tripUi.indexTripsSuccess).catch(tripUi.indexTripsFailure);
	};

	var onUpdateTrip = function onUpdateTrip(tripId) {
	  // event.preventDefault()
	  ...
	  tripApi.showTrip(tripId).then(tripUi.showTripSuccess).catch(tripUi.showTripFailure);
	};

	var onSaveUpdatedTrip = function onSaveUpdatedTrip(event) {
	  event.preventDefault
	  // console.log('(trips/events.js) onSaveUpdatedTrip')

	  ();var tripId = store.trip.id;
	  var data = getFormFields(event.target
	  ...
	  // If the first character of ALL `trip` fields have data, check for valid entered date
	    ...
	    if (data.trip.numberOfRuns > 60) {
	      // User entered a number for `numberOfRuns` that existed the max, log error
	      //  and return.
	      tripUserText.updateMyTripNumberOfMaxRunsReachedError();
	      return;
	    }
	    ...
	    ();tripApi.updateTrip(tripId, data).then(tripUi.updateTripSuccess).catch(tripUi.updateTripFailure);
	  } else {
	...
	var onDeleteTrip = function onDeleteTrip(event) {
	  event.preventDefault
	  // console.log('(trips/events.js) onDeleteTrip ran!')

	  ();var tripId = $(this).data('id');
	  tripApi.deleteTrip(tripId).then(tripUi.deleteTripSuccess(tripId)).catch(tripUi.deleteTripFailure);
	};

	// reset history modal?
	var onHistoryClose = function onHistoryClose(event) {
	  event.preventDefault
	...
	var showLogMyTripModal = function showLogMyTripModal(event) {
	  // console.log('this should toggle log-my-trip modal')
	  $('#tempLogMyTripError').html('');
	  $('#myLogMyTripModal').modal('toggle');
	};

	var showAllMyTripsModal = function showAllMyTripsModal(event) {
	  // console.log('this should toggle show-all-my-trips modal')
	  $('#view-trip-history-footer').html('');
	  $('#myShowAllMyTripsModal').modal('toggle');
	};

	var showUpdateMyTripModal = function showUpdateMyTripModal(event) {
	...
	var addTripHandlers = function addTripHandlers() {
	...
	module.exports = {
	  addTripHandlers: addTripHandlers,
	  onSaveUpdatedTrip: onSaveUpdatedTrip
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var indexTrips = function indexTrips() {
	...
	var showTrip = function showTrip(id) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var showUpdateMyTripTemplate = __webpack_require__(17

	// const tripApi = require('./api')

	);var errorTextNoCurrentTrips = 'You have no current trips.  Please log a trip!';

	var createTripSuccess = function createTripSuccess() {
	...
	var createTripFailure = function createTripFailure(error) {
	...
	var indexTripsSuccess = function indexTripsSuccess(ajaxResponse) {
	  // console.log('(trips/ui.js) indexTripsSuccess ran!  Data is :', ajaxResponse)

	  var trips = ajaxResponse;
	  store.trips = trips;
	  ...
	  if (trips.trips.length === 0) {
	    ...
	    var displayAllTrips = __webpack_require__(38);
	    $('#display-trips').prepend(displayAllTrips(trips));
	  }
	};

	var indexTripsFailure = function indexTripsFailure(error) {
	  // console.log('(trips/ui.js) indexTripsFailure  Error is :', error)
	  console.error(error);
	};

	var showTripSuccess = function showTripSuccess(ajaxResponse) {
	  // event.preventDefault()
	  ...
	  var tripId = ajaxResponse.trip.id;
	  ...
	  ();var showUpdateMyTripHtml = showUpdateMyTripTemplate({ trip: ajaxResponse.trip });
	  $('#handlebars-update-my-trip-form').html(showUpdateMyTripHtml);
	};

	var showTripFailure = function showTripFailure(error) {
	...
	var updateTripFailure = function updateTripFailure(error) {
	  // console.log('(trips/ui.js) updateTripRatingFailure  Error is :', error)
	  console.error(error);
	};

	var deleteTripSuccess = function deleteTripSuccess(id) {
	  event.preventDefault
	...
	var deleteTripFailure = function deleteTripFailure(error) {
	  // console.log('(trips/ui.js) deleteTripFailure  Error is :', error)
	  console.error(error);
	};

	var showViewTripHistoryBtn = function showViewTripHistoryBtn() {
	...
	var hideViewTripHistoryBtn = function hideViewTripHistoryBtn() {
	...
	module.exports = {
	  ...
	  indexTripsSuccess: indexTripsSuccess,
	  indexTripsFailure: indexTripsFailure,
	  showTripSuccess: showTripSuccess,
	  showTripFailure: showTripFailure,
	  ...
	  showViewTripHistoryBtn: showViewTripHistoryBtn,
	  hideViewTripHistoryBtn: hideViewTripHistoryBtn
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<!-- Update Trip input form -->\n<div data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" id=\"updateForm"
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" class='form-group updateDiv'>\n  <form data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" id=\"update-my-trip\" class='form-group updateForm'>\n    <div class=\"form-group\">\n      <label for=\"name\">Trip Date</label>\n      <input id=\"date\" type=\"date\" name=\"trip[tripDate]\" class=\"form-control\" value=\""
	    + alias2(__default(__webpack_require__(37)).call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.tripDate : stack1),{"name":"date","hash":{},"data":data}))
	    + "\" placeholder=\"Trip Date\">\n      <label for=\"name\">Resort</label>\n      <input type=\"text\" name=\"trip[resort]\" class=\"form-control\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.resort : stack1), depth0))
	    + "\" placeholder=\"Resort\">\n      <label for=\"name\">Equipment Type</label><br>\n      <input type=\"radio\" name=\"trip[equipmentType]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.equipmentType : stack1), depth0))
	    + "\" checked> Ski<br>\n      <input type=\"radio\" name=\"trip[equipmentType]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.equipmentType : stack1), depth0))
	    + "\"> Snowboard<br>\n      <label for=\"name\">Conditions</label>\n      <input type=\"text\" name=\"trip[conditions]\" class=\"form-control\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.conditions : stack1), depth0))
	    + "\" placeholder=\"Conditions\">\n      <label for=\"name\">Number of Runs</label>\n      <input type=\"text\" name=\"trip[numberOfRuns]\" class=\"form-control\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.numberOfRuns : stack1), depth0))
	    + "\" placeholder=\"How many runs did you do?\">\n      <label for=\"name\">Favorite Run</label>\n      <input type=\"text\" name=\"trip[favoriteRun]\" class=\"form-control\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.favoriteRun : stack1), depth0))
	    + "\" placeholder=\"What was your favorite run?\">\n      <label for=\"name\">Comments</label>\n      <input type=\"text\" name=\"trip[comments]\" class=\"form-control\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.comments : stack1), depth0))
	    + "\" placeholder=\"How did it go?\"><br>\n      <div class=\"saveUpdateBtn\">\n        <button type=\"submit\" class=\"btn btn-primary btn-xs\" id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.trip : depth0)) != null ? stack1.id : stack1), depth0))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "\n  <div class=\"tripWrapper\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n    <div class=\"tripDate\" id=\"tripDate"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      Trip Date: "
	    + alias2(__default(__webpack_require__(37)).call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.tripDate : stack1),{"name":"date","hash":{},"data":data,"blockParams":blockParams}))
	    + "\n    </div>\n    <div class=\"resort\" id=\"resort"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      Resort: "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.resort : stack1), depth0))
	    + "\n    </div>\n    <div class=\"equipmentType\" id=\"equipmentType"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      Equipment Type: "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.equipmentType : stack1), depth0))
	    + "\n    </div>\n    <div class=\"conditions\" id=\"conditions"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      Conditions: "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.conditions : stack1), depth0))
	    + "\n    </div>\n    <div class=\"numberOfRuns\" id=\"numberOfRuns"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      Number of Runs: "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.numberOfRuns : stack1), depth0))
	    + "\n    </div>\n    <div class=\"favoriteRun\" id=\"favoriteRun"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      My favorite run of the day: "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.favoriteRun : stack1), depth0))
	    + "\n    </div>\n    <div class=\"comments\" id=\"comments"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      How did the day go?: "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.comments : stack1), depth0))
	    + "\n    </div>\n    <div class=\"updateAndRemoveBtn\">\n      <button class=\"setUpdateTripBtn btn btn-primary btn-xs\" id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">Update Trip</button>\n      <button class=\"removeTripFromMyTripListBtn glyphicon glyphicon-trash\" id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\"></button>\n    </div><br><hr />\n  </div>\n\n";
	  ...
	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.trips : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var logMyTripSuccess = function logMyTripSuccess() {
	  $('#tempLogMyTripError').html('');
	};

	var logMyTripError = function logMyTripError() {
	  $('#tempLogMyTripError').html('All fields are required.  Please verify all fields are filled in.');
	};

	var logMyTripFutureDateError = function logMyTripFutureDateError() {
	  $('#tempLogMyTripError').html("Invalid date selected.  Please select today's date or a previous date.");
	};

	var logMyTripNumberOfRunsNotANumberError = function logMyTripNumberOfRunsNotANumberError() {
	  $('#tempLogMyTripError').html('Number of Runs field is not a number.  Please enter a number.');
	};

	var logMyTripNumberOfMaxRunsReachedError = function logMyTripNumberOfMaxRunsReachedError() {
	  $('#tempLogMyTripError').html('Are you crazy?!?!?  Please enter your actual number of runs.');
	};

	var updateMyTripSuccess = function updateMyTripSuccess() {
	  $('#tempUpdateMyTripError').html('');
	};

	var updateMyTripError = function updateMyTripError() {
	  $('#tempUpdateMyTripError').html('All fields are required.  Please verify all fields are filled in.');
	};

	var updateMyTripFutureDateError = function updateMyTripFutureDateError() {
	  $('#tempUpdateMyTripError').html("Invalid date selected.  Please select today's date or a previous date.");
	};

	var updateMyTripNumberOfRunsNotANumberError = function updateMyTripNumberOfRunsNotANumberError() {
	  $('#tempUpdateMyTripError').html('Number of Runs field is not a number.  Please enter a number.');
	};

	var updateMyTripNumberOfMaxRunsReachedError = function updateMyTripNumberOfMaxRunsReachedError() {
	  $('#tempUpdateMyTripError').html('Are you crazy?!?!?  Please enter your actual number of runs.');
	};

	module.exports = {
	  logMyTripSuccess: logMyTripSuccess,
	  logMyTripError: logMyTripError,
	  logMyTripFutureDateError: logMyTripFutureDateError,
	  logMyTripNumberOfRunsNotANumberError: logMyTripNumberOfRunsNotANumberError,
	  logMyTripNumberOfMaxRunsReachedError: logMyTripNumberOfMaxRunsReachedError,
	  updateMyTripSuccess: updateMyTripSuccess,
	  updateMyTripError: updateMyTripError,
	  updateMyTripFutureDateError: updateMyTripFutureDateError,
	  updateMyTripNumberOfRunsNotANumberError: updateMyTripNumberOfRunsNotANumberError,
	  updateMyTripNumberOfMaxRunsReachedError: updateMyTripNumberOfMaxRunsReachedError
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "af97078fab75652c41ecea37e2a0b11f.jpg";
